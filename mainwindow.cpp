    ...
    , ui(new Ui::MainWindow)
{
    ...
    connect(thread1,SIGNAL(MySignal(QString)),this,SLOT(onGet()));
    thread1->start();

    connect(mgr1, &QNetworkAccessManager::finished, [=](QNetworkReply * reply){
        if(reply->error() == QNetworkReply::NoError){
            QByteArray bytes = reply->readAll();

            QJsonObject json;
            json = QJsonDocument::fromJson(bytes).object();
            //qDebug()<<json.value("light").toBool();
            if(json.value("light").toBool()==false){
                QString c="c";
                if(!QString::compare(ui->label_3->text(),"主灯目前状态：打开"))
                mySerialPort->write(c.toLocal8Bit().data());
                ui->label_3->setText("主灯目前状态：关闭");
            }else{
                QString c="o";
                if(!QString::compare(ui->label_3->text(),"主灯目前状态：关闭"))
                mySerialPort->write(c.toLocal8Bit().data());
                ui->label_3->setText("主灯目前状态：打开");
            }
            if(temp.toFloat()<ui->lineEdit->text().toFloat()){
                //ui->label_6->hide();
                if(json.value("alarm").toBool()==false){
                    QString c="2";
                    if(!QString::compare(ui->label_4->text(),"警报目前状态：打开"))
                    mySerialPort->write(c.toLocal8Bit().data());
                    ui->label_4->setText("警报目前状态：关闭");
                }else{
                    QString c="1";
                    if(!QString::compare(ui->label_4->text(),"警报目前状态：关闭"))
                    mySerialPort->write(c.toLocal8Bit().data());
                    ui->label_4->setText("警报目前状态：打开");
                }
                if(!QString::compare(ui->label_4->text(),"警报目前状态：打开") && json.value("alarm").toBool())
                ui->label_4->setText("警报目前状态：关闭");
            }else {
               // ui->label_6->show();
                ui->label_4->setText("警报目前状态：打开");
            }
            if(json.value("force").toBool()==true){
                QString c="3";
                mySerialPort->write(c.toLocal8Bit().data());
                //ui->label_6->hide();
                ui->label_4->setText("警报目前状态：关闭");
            }else{
                if(json.value("alarm").toBool()==true){
                    QString c="1";
                    ui->label_4->setText("警报目前状态：打开");
                    mySerialPort->write(c.toLocal8Bit().data());
                }
...
void MainWindow::serialPortRead_Slot()
{
    ...
    if(ui->chkRec->checkState() == false){
        ...
        txtBuf += recBuf;
        //qDebug(recBuf);
        for (int i=0;i<txtBuf.size();i++) {
            //qDebug()<<(int)recBuf[i];
            if(txtBuf[i]=='k')
                flag=true;
            else if(txtBuf[i]=='p'){
                flag=false;
                if(_temp.size()==5&&_humi.size()==5){
                    temp=_temp;
                    humi=_humi;
                    QJsonObject obj;
                    obj["t"] = temp;
                    obj["h"] = humi;
                    if(!QString::compare(temp,_temp)||!QString::compare(humi,_humi)){
                         txtBuf="";
                        post("http://81.70.250.198:12345",obj);

                        ui->label->setText("温度："+temp+"℃");
                        ui->label_2->setText("湿度："+humi+"%");
                    }
                }
                _temp="";
                _humi="";
            }
            else if(!flag&&txtBuf[i]!='\r'&&txtBuf[i]!='\n'&&txtBuf[i]!='\0'){
                _temp+=recBuf[i];
                _temp.replace(h0,"");
        ...
        //qDebug()<<temp<<_temp<<" "<<humi<<QString::compare(temp,_temp);
...
void MainWindow::post(QString urls, QJsonObject obj)
{
...
void MainWindow::on_lblBaudRate_linkActivated(QString const&){}
void MainWindow::onGet()
{
    mgr1->get(QNetworkRequest(QUrl("http://81.70.250.198:12345/light")));
}
